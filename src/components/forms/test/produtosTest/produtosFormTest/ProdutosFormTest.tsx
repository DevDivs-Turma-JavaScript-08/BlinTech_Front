...
export default function ProdutosFormTest({ onClose }: ProdutosFormProps) {
	...
	async function buscarCategoriaPorId(id: string) {
		try {
			...
			// 👉 Atualiza direto no produto quando seleciona
			setProduto((prev) => ({
				...prev,
				categoria: { id: Number(id) } as Categoria,
			}));
		} catch (error: any) {
			if (error.toString().includes("401")) handleLogout();
		}
	...
	function atualizarEstado(e: ChangeEvent<HTMLInputElement | HTMLSelectElement>) {
		...
		setProduto({
			...produto,
			[name]: name === "valorProduto" || name === "tempoUso" ? Number(value) : value,
			categoria: categoria,
			usuario: usuario,
		});
	}
